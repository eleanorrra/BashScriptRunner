...
from internal.parsing_error import ParsingError
...
class Parser:
    ...
    def _get_line_info(line, i):
        ...
        if nesting != len(spaces) // 4:
            raise ParsingError(i+1, "wrong indent")
    ...
    def parse(self):
        for i, line in enumerate(self.code_lines):
            ...
            if nesting == 0 and self.root_node is not None:
                raise ParsingError(i+1, "only one root task allowed")
            ...
            if nesting < self.last_nesting:
                ...
                if not isinstance(self.last_node, Group):
                    raise ParsingError(i, "Group head should be either 'series' or 'parallel'")
